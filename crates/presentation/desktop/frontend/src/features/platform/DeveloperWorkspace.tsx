...
export function DeveloperWorkspace({ onAction }: { onAction: (message: string) => void }) {
  ...
  const [visualVariant, setVisualVariant] = useState<'Original'|'Variant A'|'Variant B'>('Original')
  const [compareView, setCompareView] = useState<'single'|'split'>('single')
        ...
        <div className="developer-pro-grid">
          <Panel title="Visual design mode">
            ...
            <div className="developer-browser-frame">
              ...
              <div className={compareView === 'split' ? 'developer-browser-frame__canvas developer-browser-frame__canvas--split' : 'developer-browser-frame__canvas'}>
                {appshotAttached && <div className="developer-appshot-chip"><Icon name="archive" size={12}/><span><strong>Appshot attached</strong><small>Screenshot + visible text context</small></span></div>}<div className="developer-browser-mock"><div className="developer-visual-variant-bar"><span>{visualVariant}</span><div><button type="button" className={compareView==='single'?'studio-button studio-button--active':'studio-button'} onClick={()=>setCompareView('single')}>Single</button><button type="button" className={compareView==='split'?'studio-button studio-button--active':'studio-button'} onClick={()=>setCompareView('split')}>Before / After</button><button type="button" className="studio-button" onClick={()=>{setVisualVariant('Original');setCompareView('single');onAction('Visual variant reverted in preview')}}>Revert</button></div></div><span className="eyebrow">Live UI preview</span><h2>Point. Draw. Describe.</h2><p>Visual instructions become scoped engineering tasks without leaving the developer workspace.</p><div className="developer-mock-grid"><span>Hero</span><span>Navigation</span><span>Composer</span><span>Responsive</span></div></div></div>
            </div>
