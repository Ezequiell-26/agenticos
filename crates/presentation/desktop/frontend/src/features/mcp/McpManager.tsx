...
export default function McpManager({ onAction }: { onAction: (message: string) => void }) {
  ...
  const [reconnectPolicy, setReconnectPolicy] = useState<'Manual'|'Auto'|'Backoff'>('Auto')
  const [exposureProfile, setExposureProfile] = useState<'Read-only'|'Guarded'|'Full'>('Guarded')
    ...
    <div className="mcp-manager">
      ...
      <section className="mcp-manager__workspace">
        ...
        <div className="mcp-manager__content">
          {tab === 'Overview' && <Section title="Server overview" description="Review endpoint metadata and capability counts before the server is exposed to an agent."><div className="mcp-manager__stat-grid"><Stat label="State" value={current.state} /><Stat label="Transport" value={current.transport} /><Stat label="Tools" value={String(current.tools)} /><Stat label="Resources" value={String(current.resources)} /><Stat label="Auth" value={current.auth} /><Stat label="Risk" value={current.risk} /></div><div className="mcp-manager__capability-strip"><span>Tools</span><span>Resources</span><span>Prompts</span><span>Sampling</span><span>Progress</span></div><div className="mcp-protocol-panel"><div><span className="eyebrow">Protocol contract</span><strong>MCP capability negotiation</strong><small>Transport, protocol version and reconnect policy are presented before runtime exposure.</small></div><label>Protocol<select value={protocolVersion} onChange={e=>setProtocolVersion(e.target.value)}><option>2025-06-18</option><option>2025-03-26</option><option>2024-11-05</option></select></label><label>Reconnect<select value={reconnectPolicy} onChange={e=>setReconnectPolicy(e.target.value as typeof reconnectPolicy)}><option>Manual</option><option>Auto</option><option>Backoff</option></select></label><label>Exposure<select value={exposureProfile} onChange={e=>setExposureProfile(e.target.value as typeof exposureProfile)}><option>Read-only</option><option>Guarded</option><option>Full</option></select></label><div className="mcp-protocol-chips"><span>sampling</span><span>progress</span><span>roots</span><span>elicitation</span><span>notifications</span></div></div><div className="callout"><Icon name="shield" size={14} /><span>MCP credentials and authorization are never stored in this presentation component.</span></div></Section>}
