...
export function TaskExecutionCenter({onAction}:{onAction:(message:string)=>void}){
 ...
 const filteredTasks=displayTasks.filter(({id,state:base})=>taskFilter==='all'||state(id,base as TaskState)===taskFilter)
 const counts=displayTasks.reduce((acc,{id,state:base})=>{const key=state(id,base as TaskState); acc[key]+=1; return acc},{running:0,queued:0,blocked:0,complete:0} as Record<TaskState,number>)
