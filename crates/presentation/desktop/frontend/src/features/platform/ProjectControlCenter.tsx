...
export function ProjectControlCenter({onAction}:{onAction:(message:string)=>void}){
 ...
 const [projectSource,setProjectSource]=useState<'existing'|'scratch'|'template'|'import'>('existing')
 const [previewPort,setPreviewPort]=useState(5173)
 const [publishTarget,setPublishTarget]=useState<'private'|'internal'|'public'>('private')
 const selected=projects.find(p=>p[0]===project)??projects[0]
 ...
 return <div className="project-control">
  ...
  {tab==='overview'&&<div className="project-control__grid"><Panel title="Current workspace"><div className="project-identity"><span className="workspace-avatar">A</span><div><span className="eyebrow">{selected[0]}</span><h2>{selected[1]}</h2><p>{selected[2]} · protected branch · worktree isolation</p></div></div><div className="project-facts"><div><span>Agent profile</span><strong>Builder · Guarded</strong></div><div><span>Model route</span><strong>Auto · coder</strong></div><div><span>Context</span><strong>66.1k / 128k</strong></div><div><span>Verification</span><strong>3 pending gates</strong></div></div><div className="platform-actions"><button className="studio-button" type="button" onClick={()=>act('Open developer workspace')}>Code</button><button className="studio-button" type="button" onClick={()=>act('Open Git control')}>Git</button><button className="studio-button studio-button--active" type="button" onClick={()=>act('Open mission control')}>Mission</button></div></Panel><Panel title="Workspace health"><div className="health-list">{[['Runtime','Offline preview','Warning'],['Git','Clean preview','Ready'],['Context','Budget available','Ready'],['Policies','Fail-closed','Ready'],['Evidence','3 pending','Blocked']].map(x=><div key={x[0]}><span><strong>{x[0]}</strong><small>{x[1]}</small></span><Tag label={x[2]}/></div>)}</div></Panel></div>}
  {tab==='projects'&&(
    <div className="project-control__grid">
      <Panel title="Project registry">
        <div className="project-list">{projects.map((item)=><button type="button" key={item[0]} className={project===item[0]?'project-row project-row--active':'project-row'} onClick={()=>setProject(item[0])}><span><strong>{item[0]}</strong><small>{item[1]} · {item[2]}</small></span><Tag label={item[3]}/><Icon name="chevron-right" size={12}/></button>)}</div>
        <div className="platform-actions"><button className="studio-button" type="button" onClick={()=>act('Create project')}>New project</button><button className="studio-button" type="button" onClick={()=>act('Link project')}>Link</button></div>
      </Panel>
      <Panel title="Project bootstrap">
        <div className="project-source-grid">{(['existing','scratch','template','import'] as const).map((source)=><button key={source} type="button" aria-pressed={projectSource===source} className={projectSource===source?'project-source-card project-source-card--active':'project-source-card'} onClick={()=>setProjectSource(source)}><strong>{source==='existing'?'Existing repository':source==='scratch'?'Start from scratch':source==='template'?'Use template':'Import package'}</strong><small>{source==='existing'?'Connect Git and keep the current tree':source==='scratch'?'Prompt first, create the repo later':'Start with a known project shape'}</small></button>)}</div>
        <div className="project-publish-bar"><div><span className="eyebrow">Live preview & publish</span><strong>Preview port {previewPort}</strong><small>Port-forwarded application preview before publishing.</small></div><label>Port<input type="number" min={1} max={65535} value={previewPort} onChange={e=>setPreviewPort(Math.max(1,Math.min(65535,Number(e.target.value)||1)))} /></label><select value={publishTarget} onChange={e=>setPublishTarget(e.target.value as typeof publishTarget)} aria-label="Publish visibility"><option value="private">Private</option><option value="internal">Internal</option><option value="public">Public</option></select><button className="studio-button" type="button" onClick={()=>act('Open live preview on port '+previewPort)}>Preview</button><button className="studio-button studio-button--active" type="button" onClick={()=>act('Publish '+project+' to '+publishTarget)}>Publish</button></div>
      </Panel>
    </div>
  )}
