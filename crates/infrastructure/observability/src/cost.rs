...
impl CostLedger {
    ...
    pub async fn open(database_url: &str) -> Result<Self, String> {
        let pool = SqlitePool::connect(database_url).await.map_err(|e| format!("cost ledger connection failed: {e}"))?;
    ...
    pub async fn set_pricing(&self, pricing: TokenPricing) -> Result<(), String> {
        if pricing.provider_id.trim().is_empty() || pricing.model_id.trim().is_empty() || !pricing.usd_per_million_tokens.is_finite() || pricing.usd_per_million_tokens < 0.0 {
    ...
    pub async fn record(&self, request_id: &str, provider_id: &str, model_id: &str, tokens_used: u64, recorded_at: u64) -> Result<UsageRecord, String> {
        let cost_usd = (tokens_used as f64 / 1_000_000.0) * self.price(provider_id, model_id).await?;
        let record = UsageRecord { request_id: request_id.to_string(), provider_id: provider_id.to_string(), model_id: model_id.to_string(), tokens_used, cost_usd, recorded_at };
    ...
    pub async fn list_recent(&self, limit: usize) -> Result<Vec<UsageRecord>, String> {
        ...
        Ok(rows.into_iter().map(|r| UsageRecord { request_id:r.0, provider_id:r.1, model_id:r.2, tokens_used:r.3.max(0) as u64, cost_usd:r.4, recorded_at:r.5.max(0) as u64 }).collect())
    }
}
...
mod tests {
    ...
    async fn ledger_records_cost_idempotently() {
        ...
        ledger.set_pricing(TokenPricing { provider_id:"p".into(), model_id:"*".into(), usd_per_million_tokens:2.0 }).await.unwrap();
