  ...
  return (
    <div className="prompt-studio">
      ...
      <section className="prompt-studio__main">
        <header className="prompt-studio__head"><div><span className="eyebrow">{current.category}</span><h2>{current.name}</h2><small>prompt://{current.id}/{current.version}</small></div><div className="studio-header__actions"><button className="studio-button" type="button" onClick={async()=>{try{const key=`prompt://${current.id}/${current.version}/copy-${Date.now()}`;await runtime.memory.upsert({namespace:'prompt-studio',key,value:text,tags:['prompt',current.category]});onAction('Prompt copy persisted in runtime memory')}catch(error){onAction(error instanceof Error?error.message:'Prompt duplication failed')}}}><Icon name="code" size={13} /> Duplicate</button><button className="studio-button studio-button--active" type="button" onClick={async()=>{try{await runtime.memory.upsert({namespace:'prompt-studio',key:`prompt://${current.id}/${current.version}`,value:text,tags:['prompt',current.category]});onAction('Prompt saved to runtime memory')}catch(error){onAction(error instanceof Error?error.message:'Prompt save failed')}}}><Icon name="check" size={13} /> Save</button></div></header>
        ...
        <div className="prompt-studio__content">
          ...
          {tab === 'Test' && <div className="prompt-test"><div className="prompt-test__toolbar"><span>Test input</span><button className="studio-button studio-button--active" type="button" onClick={async(event)=>{const button=event.currentTarget;button.disabled=true;try{const status=await runtime.status.get();const model=status.model && status.model !== 'Runtime-selected model' ? status.model : (await runtime.models.list())[0]?.model_id;if(!model)throw new Error('No runtime model is configured for prompt testing.');const input=(event.currentTarget.parentElement?.nextElementSibling as HTMLTextAreaElement | null)?.value ?? 'Inspect the current frontend and identify the next safe implementation slice.';const result=await runtime.models.execute(model,text+'\n\nTest input:\n'+input);onAction('Prompt test completed with '+model+': '+(result.response?.slice(0,180) ?? 'runtime response received'))}catch(error){onAction(error instanceof Error?error.message:'Prompt test failed')}finally{button.disabled=false}}}><Icon name="play" size={13} /> Run test</button></div><textarea defaultValue="Inspect the current frontend and identify the next safe implementation slice." aria-label="Prompt test input" /><div className="prompt-test__result"><span>Runtime output</span><strong>Live model execution</strong><small>The test now uses the configured runtime model and returns real output through the backend.</small></div></div>}
        </div>
      </section>
    </div>
  )
