...
use super::{Capability, CapabilityId, CapabilityOrigin, CapabilityStatus, BrainError, License};
...
impl CapabilityRegistry {
    ...
    pub async fn register(&self, capability: Capability) -> Result<(), BrainError> {
        ...
        // Update indexes
        {
            ...
            index_by_type.entry(type_key).or_insert_with(Vec::new).push(capability.id.clone());
        }
        
        {
            let mut index_by_license = self.index_by_license.write().await;
            index_by_license.entry(capability.license.clone()).or_insert_with(Vec::new).push(capability.id.clone());
        }
        
        {
            let mut index_by_status = self.index_by_status.write().await;
            index_by_status.entry(capability.status.clone()).or_insert_with(Vec::new).push(capability.id.clone());
        }
        ...
        // Update dependency graph
        {
            ...
            for dep_id in &capability.dependencies {
                reverse_graph.entry(dep_id.clone()).or_insert_with(Vec::new).push(capability.id.clone());
            }
        }
    ...
    pub async fn update_status(&self, id: &CapabilityId, status: CapabilityStatus) -> Result<(), BrainError> {
        ...
        index.entry(status).or_insert_with(Vec::new).push(id.clone());
    ...
    pub async fn remove(&self, id: &CapabilityId) -> Result<(), BrainError> {
        ...
        if !dependents.is_empty() {
            return Err(BrainError::DependencyConflict(
                format!("Cannot remove capability {} - {} capabilities depend on it", id, dependents.len())
            ));
        }

        let mut registry = self.capabilities.write().await;
        let capability = registry.remove(id)
            .ok_or_else(|| BrainError::CapabilityNotFound(id.clone()))?;
...
mod tests {
    ...
    async fn test_update_status() {
        ...
        registry.update_status(&"test-capability".to_string(), CapabilityStatus::Accepted).await.unwrap();
    ...
    async fn test_stats() {
        ...
        registry.register(create_test_capability("cap1")).await.unwrap();
        registry.register(create_test_capability("cap2")).await.unwrap();
    ...
    async fn test_remove_capability() {
        ...
        registry.register(capability).await.unwrap();
        registry.remove(&"test-capability".to_string()).await.unwrap();
