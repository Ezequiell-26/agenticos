...
export default function PromptStudio({ onAction }: { onAction: (message: string) => void }) {
  ...
  const [evalModel, setEvalModel] = useState('Auto route')
  const [evalSuite, setEvalSuite] = useState('Frontend engineering')
  const [regressionGate, setRegressionGate] = useState(true)
    ...
    <div className="prompt-studio">
      ...
      <section className="prompt-studio__main">
        ...
        <div className="prompt-studio__content">
          ...
          {tab === 'Test' && <div className="prompt-test"><div className="prompt-test__toolbar"><span>Test input</span><select value={evalSuite} onChange={e=>setEvalSuite(e.target.value)} aria-label="Evaluation suite"><option>Frontend engineering</option><option>Reasoning safety</option><option>Tool calling</option><option>Long context</option></select><select value={evalModel} onChange={e=>setEvalModel(e.target.value)} aria-label="Evaluation model"><option>Auto route</option><option>GPT-5.3-Codex</option><option>Claude Sonnet</option><option>Gemini</option><option>Local</option></select><button type="button" className={regressionGate?'studio-button studio-button--active':'studio-button'} aria-pressed={regressionGate} onClick={()=>setRegressionGate(v=>!v)}>Regression gate</button><button className="studio-button studio-button--active" type="button" onClick={() => onAction('Prompt test executed in preview')}><Icon name="play" size={13} /> Run test</button></div><textarea defaultValue="Inspect the current frontend and identify the next safe implementation slice." aria-label="Prompt test input" /><div className="prompt-eval-metrics"><div><span>Suite score</span><strong>94%</strong></div><div><span>Pass rate</span><strong>17/18</strong></div><div><span>Regression</span><strong>{regressionGate?'Blocked on failures':'Advisory'}</strong></div><div><span>Model</span><strong>{evalModel}</strong></div></div><div className="prompt-test__result"><span>Preview output · {evalSuite}</span><strong>Structured response staged</strong><small>Use the real model runtime to render actual output, traces and citations.</small></div></div>}
        </div>
      </section>
    </div>
  )
