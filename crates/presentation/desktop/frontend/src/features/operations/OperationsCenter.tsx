  ...
  return (
    <div className="operations-center">
      ...
      <div className="operations-content">
        {tab === 'Health' && <><div className="operation-metric-grid">{[['Runtime','Ready'],['Providers','4 configured'],['Gateway','Stopped'],['Memory','Bounded']].map(([label,value]) => <div className="metric-card" key={label}><span>{label}</span><strong>{value}</strong><small>presentation state</small></div>)}</div><section className="surface-block"><div className="operations-livebar"><div><span className="eyebrow">Long-running control</span><strong>{heartbeat?'Heartbeat healthy':'Heartbeat paused'}</strong><small>{autoResume?'Cross-device resume armed':'Manual resume only'} · 2 pending approvals</small></div><button type="button" className={heartbeat?'studio-button studio-button--active':'studio-button'} aria-pressed={heartbeat} onClick={()=>setHeartbeat(v=>!v)}>Heartbeat</button><button type="button" className={autoResume?'studio-button studio-button--active':'studio-button'} aria-pressed={autoResume} onClick={()=>setAutoResume(v=>!v)}>Auto resume</button><button type="button" className={alertsOnly?'studio-button studio-button--active':'studio-button'} aria-pressed={alertsOnly} onClick={()=>setAlertsOnly(v=>!v)}>Alerts only</button><button type="button" className="studio-button studio-button--active" onClick={()=>onAction('Remote operations inbox opened in preview')}>Ops inbox</button></div><div className="operations-timeline">{[['09:41','heartbeat','Worker healthy'],['09:39','approval','Terminal approval pending'],['09:37','resume','Session resumed from checkpoint'],['09:33','alert','Provider fallback engaged']].filter(item=>!alertsOnly||item[1]==='approval'||item[1]==='alert').map(item=><div key={item[0]+item[1]}><span className="mono-text">{item[0]}</span><span className="state-pill state-pill--pending">{item[1]}</span><strong>{item[2]}</strong><button type="button" onClick={()=>onAction(item[1]+' detail opened in preview')}>Inspect</button></div>)}</div><div className="surface-block__heading"><span>Operations</span><span className="mono-text">safe actions</span></div>{operations.map(([name,detail,state]) => <div className="operation-row" key={name}><div><strong>{name}</strong><span>{detail}</span></div><span className="state-pill state-pill--pending">{state}</span><button className={runningOp === name ? 'studio-button studio-button--active' : 'studio-button'} type="button" onClick={() => run(name)}>{runningOp === name ? 'Running…' : 'Open'}</button></div>)}</section></>}
