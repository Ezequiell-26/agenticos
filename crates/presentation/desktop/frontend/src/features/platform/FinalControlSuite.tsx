...
export function FinalControlSuite({mode,onAction}:{mode:FinalControlMode;onAction:(message:string)=>void}){
  ...
  return <div className="final-suite">
    ...
    <div id="final-suite-tabpanel" className="final-suite__metrics" role="tabpanel" aria-labelledby={'final-suite-tab-' + active.toLowerCase()} tabIndex={0}>{suite.metrics.map(([label,value])=><MetricCard key={label} label={label} value={liveMetric(label,value)} sub={syncing ? 'Syncing runtime' : liveAuditCount !== null ? 'Runtime telemetry' : 'UI baseline'}/>)}</div>
