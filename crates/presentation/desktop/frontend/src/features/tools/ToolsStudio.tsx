  ...
  return (
    <div className="tools-studio">
      ...
      <section className="tools-studio__detail">
        ...
        <div className="tool-playground"><div className="tool-playground__head"><div><span className="eyebrow">Tool Playground</span><strong>Dry-run {current.name}</strong><small>Validate arguments and inspect the expected contract before a runtime call.</small></div><button type="button" className={playgroundOpen?'studio-button studio-button--active':'studio-button'} aria-pressed={playgroundOpen} onClick={()=>setPlaygroundOpen(v=>!v)}>{playgroundOpen?'Close':'Open'}</button></div>{playgroundOpen && <div className="tool-playground__body"><label>Arguments<textarea value={toolInput} onChange={e=>setToolInput(e.target.value)} aria-label="Tool arguments" /></label><div><span className="eyebrow">Preview result</span><pre>{'{\n  "status": "ready",\n  "approval": "'+(current.risk==='Critical'?'required':'policy')+'",\n  "mutation": '+(current.risk==='Critical'?'false':'scoped')+'\n}'}</pre></div><div className="platform-actions"><button className="studio-button" type="button" onClick={()=>onAction(current.name+' arguments validated in preview')}>Validate args</button><button className="studio-button studio-button--active" type="button" onClick={()=>onAction(current.name+' dry run staged in preview')}><Icon name="play" size={13}/> Dry run</button></div></div>}</div><div className="platform-actions"><button className="studio-button" type="button" onClick={() => onAction(current.name + ' test run opened in preview')}><Icon name="play" size={13} /> Test tool</button><button className="studio-button" type="button" onClick={() => onAction(current.name + ' schema copied')}><Icon name="copy" size={13} /> Copy schema</button><button className="studio-button studio-button--active" type="button" onClick={() => onAction(current.name + ' policy editor opened in preview')}><Icon name="shield" size={13} /> Edit policy</button></div>
      </section>
    </div>
  )
}
