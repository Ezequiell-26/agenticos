...
export function KnowledgeStudio({ onAction }: { onAction: (message:string)=>void }) {
 ...
 const [rerank,setRerank]=useState(true);const [retrievalQuery,setRetrievalQuery]=useState('How does AgentiCOS verify frontend changes safely?');const [topK,setTopK]=useState(8);const [hybridWeight,setHybridWeight]=useState(60)
 ...
 return <div className="studio-shell">
  ...
  {activeTab==='retrieval' && <div className="knowledge-layout"><Panel title="Retrieval pipeline"><div className="knowledge-retrieval-debug"><label>Query<input value={retrievalQuery} onChange={e=>setRetrievalQuery(e.target.value)}/></label><label>Top K<input type="range" min={3} max={20} value={topK} onChange={e=>setTopK(Number(e.target.value))}/><span>{topK} chunks</span></label><label>Hybrid weight<input type="range" min={0} max={100} value={hybridWeight} onChange={e=>setHybridWeight(Number(e.target.value))}/><span>{hybridWeight}% lexical · {100-hybridWeight}% semantic</span></label></div><div className="strategy-stack"><div><span>Query rewrite</span><strong>Enabled</strong></div><div><span>Hybrid search</span><strong>Vector + lexical</strong></div><div><span>Reranking</span><strong>{rerank?'Enabled':'Disabled'}</strong></div><div><span>Context packing</span><strong>Budget aware</strong></div></div><div className="knowledge-retrieval-results">{[['FRONTEND-ARCHITECTURE-MAP.md','0.94','policy + architecture'],['FRONTEND-CHANGELOG.md','0.91','verification + history'],['DeveloperWorkspace.tsx','0.88','agent workflow'],['SecurityCenter.tsx','0.84','approval boundary']].slice(0,Math.min(topK,4)).map(item=><div key={item[0]}><span><strong>{item[0]}</strong><small>{item[2]}</small></span><b>{item[1]}</b><button type="button" onClick={()=>onAction('Retrieval chunk '+item[0]+' opened in preview')}>Inspect</button></div>)}</div><div className="knowledge-retrieval-metrics"><div><span>Rerank</span><strong>{rerank?'Enabled':'Disabled'}</strong></div><div><span>Context packed</span><strong>3.8k</strong></div><div><span>Citation coverage</span><strong>100%</strong></div><div><span>Grounding</span><strong>94%</strong></div></div><div className="platform-actions"><button className={rerank?'studio-button studio-button--active':'studio-button'} type="button" onClick={()=>setRerank(v=>!v)}>Toggle reranker</button><button className="studio-button" type="button" onClick={()=>onAction('Retrieval test opened in preview')}>Test retrieval</button><button className="studio-button studio-button--active" type="button" onClick={()=>onAction('Context packet preview opened')}>Preview context</button></div></Panel><Panel title="Provenance"><div className="knowledge-citations">{citations.map(([title,path,score])=><button type="button" className="citation-row" key={path} onClick={()=>onAction('Citation '+title+' opened in preview')}><div><strong>{title}</strong><span>{path}</span></div><b>{score}</b></button>)}</div><div className="callout"><Icon name="shield" size={14}/><span>Grounded responses should preserve source IDs and retrieval scores so the backend can attach verifiable citations later.</span></div></Panel></div>}
