...
export function FrontendCompletenessStudio({ mode, onAction }: Props) {
  ...
  if (mode === 'skills') return <Shell><Header eyebrow="Capability registry" title="Skills Studio" detail="Install, version, scope, trust, test and update procedural capabilities without coupling the UI to runtime installation." onAction={onAction} />
    ...
    <div className="fc-card-grid">{filteredSkills.map(([id, title, version, trust, tools]) => <button type="button" key={id} className={selected === id ? 'fc-card fc-card--active' : 'fc-card'} onClick={() => setSelected(id)}><div className="fc-card__top"><Icon name="spark" size={17} /><Tag label={trust} /></div><strong>{title}</strong><span>{id} · v{version}</span><small>{tools}</small><div className="fc-progress"><i style={{ width: trust === 'Local' ? '64%' : '94%' }} /></div></button>)}</div>
  ...
  if (mode === 'providers') return <Shell><Header eyebrow="Model control plane" title="Provider & Model Center" detail="Catalog models, quotas, pricing metadata, latency, health, capabilities and routing policies." onAction={onAction} />
    ...
    <div className="fc-card-grid">{providers.map(([id, detail, health, role, context]) => <button type="button" key={id} className={selected === id ? 'fc-card fc-card--active' : 'fc-card'} onClick={() => setSelected(id)}><div className="fc-card__top"><Icon name="network" size={17} /><Tag label={health} /></div><strong>{id}</strong><span>{detail}</span><small>{role} · {context} context</small><div className="fc-health"><i style={{ width: health === 'Healthy' ? '94%' : health === 'Degraded' ? '62%' : '14%' }} /></div></button>)}</div>
  ...
  if (mode === 'computer') return <Shell><Header eyebrow="Computer use" title="Computer Use Recorder" detail="Record, inspect, replay and verify desktop actions with checkpoints and explicit approval boundaries." onAction={onAction} />
    <div className="fc-card-grid">{recordings.map(([id, title, steps, state]) => <button type="button" key={id} className={selected === id ? 'fc-card fc-card--active' : 'fc-card'} onClick={() => setSelected(id)}><div className="fc-card__top"><Icon name="layout" size={17} /><Tag label={state} /></div><strong>{title}</strong><span>{id}</span><small>{steps}</small></button>)}</div>
