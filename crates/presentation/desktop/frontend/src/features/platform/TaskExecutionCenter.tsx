...
export function TaskExecutionCenter({onAction}:{onAction:(message:string)=>void}){
 ...
 return <div className="task-execution">
  <header className="task-execution__hero"><div><span className="eyebrow">Execution workspace</span><h1>Task Execution Center</h1><p>Operate task queues, terminals, processes, approvals, logs and evidence from one bounded execution workspace.</p></div><div className="task-execution__actions"><Tag label="Preview"/><button className="studio-button" type="button" onClick={()=>act('Pause all tasks')}><Icon name="stop" size={13}/>Pause all</button><button className="studio-button studio-button--active" type="button" onClick={()=>act('Resume queue')}><Icon name="play" size={13}/>Resume queue</button></div></header>
  <div className="platform-metrics"><MetricCard label="Tasks" value="2 active" sub="1 queued · 1 blocked"/><MetricCard label="Terminals" value="3" sub="2 active processes"/><MetricCard label="Approvals" value="1" sub="Execution gate pending"/><MetricCard label="Artifacts" value="4" sub="Evidence package in progress"/></div>

  <div className="task-execution__layout">
   <Panel title="Task queue">
    <div className="task-execution__task-list">{initialTasks.map(([id,title,owner,base,branch])=><button type="button" key={id} className={selected===id?'task-row task-row--active':'task-row'} onClick={()=>setSelected(id)}><span className={'task-state task-state--'+state(id,base as TaskState)}><Icon name={state(id,base as TaskState)==='complete'?'check':state(id,base as TaskState)==='blocked'?'lock':state(id,base as TaskState)==='running'?'activity':'clock'} size={12}/></span><span><strong>{title}</strong><small>{id} · {owner} · {branch}</small></span><span className="mono-text">{state(id,base as TaskState)}</span></button>)}</div>
    <div className="platform-actions"><button className="studio-button" type="button" onClick={()=>act('Open task plan')}>Plan</button><button className="studio-button" type="button" onClick={()=>act('Delegate task')}>Delegate</button><button className="studio-button studio-button--active" type="button" onClick={()=>{setTaskState(v=>({...v,[selected]:'running'}));act('Start task')}}><Icon name="play" size={12}/>Start</button></div>
   </Panel>

   <Panel title="Selected task">
    <div className="task-execution__task-header"><div><span className="eyebrow">{current[2]}</span><h2>{current[1]}</h2><p>{current[0]} · isolated task scope · {current[4]}</p></div><span className="state-pill state-pill--pending">{state(current[0],current[3] as TaskState)}</span></div>
    ...
    <div className="task-execution__steps">{['Scope resolved','Plan accepted','Implementation','Verification','Evidence'].map((step,index)=><div key={step} className={index<2?'task-step task-step--done':'task-step'}><span>{index<2?'✓':index+1}</span><strong>{step}</strong></div>)}</div>
    <div className="platform-actions"><button className="studio-button" type="button" onClick={()=>act('Retry selected task')}>Retry</button><button className="studio-button" type="button" onClick={()=>act('Open checkpoint')}><Icon name="history" size={12}/>Checkpoint</button><button className="studio-button studio-button--active" type="button" onClick={()=>act('Stop selected task')}>Stop</button></div>
   </Panel>
  </div>
