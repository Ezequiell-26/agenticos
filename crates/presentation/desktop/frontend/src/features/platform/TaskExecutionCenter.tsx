...
export function TaskExecutionCenter({onAction}:{onAction:(message:string)=>void}){
 ...
 const runPlan=async()=>{
   try{await runtime.reasoning.plan(current[1]);onAction('Task plan generated by runtime')}catch(error){onAction(error instanceof Error?error.message:'Task planning failed')}
 }
 const createRuntimeTerminal=async()=>{
   const grantId=window.prompt('Capability grant_id for terminal execution')?.trim()
   if(!grantId) return
   const command=window.prompt('Command to run in the new terminal','')?.trim()
   if(!command) return
   try{const created=await runtime.terminal.create({command,grant_id:grantId});setTerminal(typeof created.terminal_id==='string'?created.terminal_id:terminal);onAction('Terminal created in runtime')}catch(error){onAction(error instanceof Error?error.message:'Terminal creation failed')}
 }
 const runCommand=async()=>{
   const grantId=window.prompt('Capability grant_id for process execution')?.trim()
   if(!grantId) return
   const command=window.prompt('Command to execute','')?.trim()
   if(!command) return
   try{await runtime.tools.execute({session_id:selected,grant_id:grantId,command});onAction('Command executed through runtime tool service')}catch(error){onAction(error instanceof Error?error.message:'Command execution failed')}
 }
 const requestApproval=async()=>{
   const runId=selectedRuntimeJob && typeof liveJobs.find((job)=>job.job_id===selected)?.run_id==='string' ? String(liveJobs.find((job)=>job.job_id===selected)?.run_id) : window.prompt('Run ID for approval')?.trim()
   if(!runId) return
   try{await runtime.approvals.create({run_id:runId,action:'task.execution',resource:selected});onAction('Approval request created in runtime')}catch(error){onAction(error instanceof Error?error.message:'Approval request failed')}
 }
 const checkpoint=async()=>{
   const runId=selectedRuntimeJob && typeof liveJobs.find((job)=>job.job_id===selected)?.run_id==='string' ? String(liveJobs.find((job)=>job.job_id===selected)?.run_id) : ''
   if(!runId){onAction('Select a runtime job with a run_id to create a checkpoint');return}
   try{await runtime.runs.snapshot(runId);onAction('Runtime checkpoint captured')}catch(error){onAction(error instanceof Error?error.message:'Checkpoint failed')}
 }
 ...
 return <div className="task-execution">
  ...
  <div className="task-execution__layout">
   ...
   <Panel title="Selected task">
    ...
    <div className="platform-actions"><button className="studio-button" type="button" onClick={()=>transitionSelected('running','Retry selected task')}>Retry</button><button className="studio-button" type="button" onClick={()=>void checkpoint()}><Icon name="history" size={12}/>Checkpoint</button><button className="studio-button studio-button--active" type="button" onClick={()=>transitionSelected('blocked','Stop selected task')}>Stop</button></div>
   </Panel>
  </div>
  ...
  <div className="task-execution__layout task-execution__layout--lower">
   <Panel title="Terminal sessions">
    ...
    <div className="terminal-preview"><div className="terminal-preview__bar"><span>{terminal}</span><Tag label="sandbox"/><button className="icon-button" aria-label="Clear terminal" type="button" onClick={()=>onAction('Terminal output is runtime-managed')}><Icon name="x" size={13}/></button></div><pre>$ cargo check -p agenticos-api-server
...
<span>waiting for runtime verification evidence...</span></pre></div>
    <div className="platform-actions"><button className="studio-button" type="button" onClick={()=>void runCommand()}>Run command</button><button className="studio-button" type="button" onClick={()=>void createRuntimeTerminal()}>New terminal</button><button className="studio-button studio-button--active" type="button" onClick={()=>void requestApproval()}><Icon name="lock" size={12}/>Approval</button></div>
   </Panel>
