...
The active operation is the backend architecture refactor and runtime hardening. The current main branch includes workflow/subagent execution, capability-gated approvals/tools, multi-provider routing and streaming, durable scheduler retries/lease recovery, ranked memory retrieval, optional browser automation, GitHub write/delete operations, request correlation, sandbox hardening, A2A and CQRS run projection. The backend now includes durable retry backoff and lease recovery, ranked persistent memory retrieval, capability-gated GitHub writes, an optional real browser adapter, request correlation, provider capability routing and a non-empty CQRS run projection.
